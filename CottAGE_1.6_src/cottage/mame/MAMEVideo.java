 ...
 */
public class MAMEVideo implements VideoEmulator, Vh_convert_color_proms,
		Eof_callback, Vh_start, Vh_stop, Vh_refresh {
	...
	protected static final int PALETTE_COLOR_CACHED = 2; /*
														 * This color is cached
														 * in temporary bitmaps
														 */
	/*
	 * palette_recalc() will try to use always the same pens for the used
	 * colors;
	 */
	...
	public TileMap tilemap_create(Get_tile_info tile_get_info,
			int get_memory_offset, int type, int tile_width, int tile_height,
			int num_cols, int num_rows) {
		return new TileMap(tile_get_info, get_memory_offset, type, tile_width,
				tile_height, num_cols, num_rows);
	}
	...
	public void tilemap_draw(BitMap dest, int[] cliprect, TileMap tilemap,
				...
				if (transparent) {
					for (k = 0; k < tile_height; k++) {
						for (l = 0; l < tile_width;) {
							...
							if (tdata != pen)
								pixels[ofs + l] = palette[tile_color + tdata];
							l++;
					...
					for (k = 0; k < tile_height; k++) {
						for (l = 0; l < tile_width;) {
							tile_data = GFX[tile_code++];
							pixels[ofs + l] = palette[tile_color
									+ ((tile_data >> 4) & 0x0F)];
							l++;
							pixels[ofs + l] = palette[tile_color
									+ (tile_data & 0x0F)];
	...
	public class RRRR_GGGG_BBBB_pi implements Vh_convert_color_proms {
		...
		public void palette_init() {
			...
			System.out.println("Machine_drv_total_colors "
					+ Machine_drv_total_colors);
	...
	public class Paletteram_RRRRGGGGBBBBxxxx_split1_w implements WriteHandler {
		...
		public void write(int address, int data) {
			RAM[address] = data;
			changecolor_RRRRGGGGBBBBxxxx(address - paletteram, RAM[address]
					| (RAM[address - paletteram + paletteram_2] << 8));
		}
	}
	...
	public class Paletteram_RRRRGGGGBBBBxxxx_split2_w implements WriteHandler {
		...
		public void write(int address, int data) {
			RAM[address] = data;
			changecolor_RRRRGGGGBBBBxxxx(address - paletteram_2, RAM[address
					- paletteram_2 + paletteram]
					| (RAM[address] << 8));
		}
	}
	...
	public class Paletteram_xxxxRRRRGGGGBBBB_swap_w implements WriteHandler {
		...
		public void write(int address, int data) {
			RAM[address] = data;
			changecolor_xxxxRRRRGGGGBBBB((address - paletteram) >> 1,
					RAM[address | 1] | (RAM[address & ~1] << 8));
		}
	}
	...
	public class Paletteram_RRRRGGGGBBBBxxxx_swap_w implements WriteHandler {
		...
		public void write(int address, int data) {
			RAM[address] = data;
			changecolor_RRRRGGGGBBBBxxxx((address - paletteram) >> 1,
					RAM[address | 1] | (RAM[address & ~1] << 8));
		}
	}
	...
	public void init(MachineDriver md) {
		...
		this.color = md.col;
		this.rot = md.ROT;
		this.pixels = new int[((visible[1] + 1) - visible[0])
				* ((visible[3] + 1) - visible[2])];
		...
		case VideoConstants.ROT180:
			backBuffer = new BitMapImpl(((visible[1] + 1) - visible[0]),
					((visible[3] + 1) - visible[2]), pixels);
			break;
		case VideoConstants.ROT90:
			vX = -(visible[0]);
			vY = -(visible[2]);
		case VideoConstants.ROT270:
			backBuffer = new BitMapImpl(((visible[3] + 1) - visible[2]),
					((visible[1] + 1) - visible[0]), pixels);
			break;
		}

		bitmap = backBuffer;
		tmpbitmap = new BitMapImpl(backBuffer.getWidth(),
		...
		try {
			...
			for (int i = 0; i < gdi.length; i++) {
				...
				gfxMan[i].init(gdi[i], palette, color_table[i], 0, rot,
						md.videoFlags);
			}
		} catch (Exception e) {
		}
	...
	public void palette_set_color(int c, int r, int g, int b) {
		...
		// rgb);
		if (palette_get_color(c) != rgb)
			palette_set_color(c, rgb);
	}
	...
	protected void drawgfx(BitMap target, int type, int tile, int color,
			boolean flipx, boolean flipy, int x, int y, int transparency,
			int transcolor) {

		switch (rot) {
		case VideoConstants.ROT0:
			gfxMan[type].drawTile(target, tile, color, flipx, flipy, x + vX, y
					+ vY, transparency, transcolor);
			break;
		case VideoConstants.ROT90:
			gfxMan[type].drawTile(target, tile, color, flipy, flipx, x + vX, y
					+ vY, transparency, transcolor);
			break;
		case VideoConstants.ROT180:
			gfxMan[type].drawTile(target, tile, color, flipx, flipy, x + vX, y
	...
	protected void drawgfx(BitMap target, int type, int tile, int color,
			int flipx, int flipy, int x, int y, int transparency, int transcolor) {
		switch (rot) {
		case VideoConstants.ROT0:
			gfxMan[type].drawTile(target, tile, color, (flipx != 0),
					(flipy != 0), x + vX, y + vY, transparency, transcolor);
			break;
		case VideoConstants.ROT90:
			gfxMan[type].drawTile(target, tile, color, (flipy != 0),
					(flipx != 0), x + vX, y + vY, transparency, transcolor);
			break;
		case VideoConstants.ROT180:
			gfxMan[type].drawTile(target, tile, color, (flipx != 0),
					(flipy != 0), x + vX, y + vY, transparency, transcolor);
	...
	protected void drawgfx(BitMap target, int type, int tile, int color,
			int flipx, int flipy, int x, int y, int area, int transparency,
			int transcolor) {
		if (area == 1) {
			x += vX;
			y += vY;
		...
		case VideoConstants.ROT0:
			gfxMan[type].drawTile(target, tile, color, (flipx != 0),
					(flipy != 0), x, y, transparency, transcolor);
			break;
		case VideoConstants.ROT90:
			gfxMan[type].drawTile(target, tile, color, (flipy != 0),
					(flipx != 0), x, y, transparency, transcolor);
			break;
		case VideoConstants.ROT180:
			gfxMan[type].drawTile(target, tile, color, (flipx != 0),
					(flipy != 0), x, y, transparency, transcolor);
	 ...
	 */
	/*
	 * public void drawTile(BitMap target, int tile, int color, boolean flipx,
	 * boolean flipy, int x, int y, int transparency, int transcolor) {
	 * 
	 * switch(rot) { case GfxManager.ROT0:
	 * getTile(tile,color,transparency,transcolor).toBitMap(target, x, y, flipx,
	 * flipy, owTrans); break; case GfxManager.ROT90:
	 * getTile(tile,color,transparency,transcolor).toBitMap(target,
	 * (target.getWidth() - w - 1) - y, x, flipx, flipy, owTrans); break; case
	 * GfxManager.ROT180:
	 * getTile(tile,color,transparency,transcolor).toBitMap(target,
	 * (target.getWidth() - w - 1) - x, (target.getHeight() - h - 1) - y, flipx,
	 * flipy, owTrans); break; case GfxManager.ROT270:
	 * getTile(tile,color,transparency,transcolor).toBitMap(target, y,
	 * (target.getHeight() - h - 1) - x, flipx, flipy, owTrans); break; } }
	 */
	...
	protected void copybitmap(BitMap dest, BitMap src, int rows, int rowscroll,
			int cols, int colscroll, int area, int transparency, int transcolor) {
		src.toPixels(pixels);
	}
	 ...
	 */
	protected void copyscrollbitmap(BitMap dest, BitMap src, int rows,
			int rowscroll, int cols, int colscroll, int transparency,
			int transcolor) {
		...
		if (rows == 1 && cols == 1) {
			...
			if (((rowscroll + w) <= sw) && ((colscroll + h) <= sh)) {
				...
				// horizontal wrap
				src.toBitMap(dest, 0, 0, rowscroll, colscroll, sw - rowscroll,
						h);
				src.toBitMap(dest, sw - rowscroll, 0, rowscroll
						+ (sw - rowscroll), colscroll, w - (sw - rowscroll), h);
			} else if (((rowscroll + w) <= sw) && ((colscroll + h) > sh)) {
				// vertical wrap
				src.toBitMap(dest, 0, 0, rowscroll, colscroll, w, h
						- ((colscroll + h) - sh));
				src.toBitMap(dest, 0, h - ((colscroll + h) - sh), rowscroll, 0,
						w, (colscroll + h) - sh);
			} else {
				// horizontal/vertical wrap
				src.toBitMap(dest, sw - rowscroll, sh - colscroll, 0, 0,
						(rowscroll + w) - sw, (colscroll + h) - sh);
				src.toBitMap(dest, 0, sh - colscroll, rowscroll, 0, sw
						- rowscroll, (colscroll + h) - sh);
				src.toBitMap(dest, sw - rowscroll, 0, 0, colscroll,
						(rowscroll + w) - sw, sh - colscroll);
				src.toBitMap(dest, 0, 0, rowscroll, colscroll, sw - rowscroll,
						sh - colscroll);
			}
		}
	...
	protected final void copyscrollbitmap(BitMap target, BitMap src, int rows,
			int[] scroll, int cols, int colscroll, int[] clip,
		...
		if (cols == 0 && colscroll == 0) {
			...
			if ((rot & 1) == 0) {
				...
				tw = target.getWidth() + (clip[0] << 1); // we assume now that
															// the visible area
															// is in the middle.
				...
				for (; i < rows; i++) {
					...
					if ((x + target.getWidth()) <= sw) {
						// No wrap
						src.toBitMap(target, 0, ydst, x, ysrc,
								target.getWidth(), rowheight, tt);
					} else {
						src.toBitMap(target, 0, ydst, x, ysrc, sw - x,
								rowheight, tt);
						src.toBitMap(target, sw - x, ydst, 0, ysrc,
								target.getWidth() - (sw - x), rowheight, tt);
					}
					// }
					ysrc += rowheight;
					ydst += rowheight;
				...
				tw = target.getWidth() + (clip[2] << 1); // we assume now that
															// the visible area
															// is in the middle.
				th = target.getHeight() + (clip[0] << 1); // we assume now that
															// the visible area
															// is in the middle.
				rowheight = tw / rows;
				for (int i = 0; i < rows; i++) {
					...
					if (yt >= 0 && yt < target.getWidth()) {
						int x = -scroll[i - 2] + th; // don't know about that -2
						...
						if ((x + target.getHeight()) <= sh) {
							// No wrap
							src.toBitMap(target, yt, 0, y, x, rowheight,
									target.getHeight());
						} else {
							src.toBitMap(target, yt, 0, y, x, rowheight, sh - x);
							src.toBitMap(target, yt, sh - x, y, 0, rowheight,
									target.getHeight() - (sh - x));
						}
					}
				}
	...
	protected final void copyscrollbitmap(BitMap target, BitMap src, int cols,
			int colscroll, int rows, int[] scroll, int[] clip,
			int transparency, int transcolor) {
		if (cols == 0 && colscroll == 0) {
			...
			if ((rot & 1) == 1) {
				...
				for (int i = 0; i < rows; i++) {
					...
					if (y >= 0 && y < target.getHeight()) {
						...
						if ((x + target.getWidth()) <= sw) {
							// No wrap
							src.toBitMap(target, 0, y, x, y, target.getWidth(),
									rowheight);
						} else {
							src.toBitMap(target, 0, y, x, y, sw - x, rowheight);
							src.toBitMap(target, sw - x, y, 0, y,
									target.getWidth() - (sw - x), rowheight);
						}
					}
				}
			} else {
