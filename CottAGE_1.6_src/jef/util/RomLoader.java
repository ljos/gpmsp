...
public final class RomLoader {
	...
	private int romlist_useram[][] = new int[MAXLIST][]; /*
														 * real memory to use to
														 * store the ROM file
														 */
	private String romlist_names[] = new String[MAXLIST]; /* ROM filename to use */
	private int romlist_storeat[] = new int[MAXLIST]; /*
													 * offset in real memory
													 * where to load the ROM
													 * file
													 */
	private int romlist_length[] = new int[MAXLIST]; /*
													 * length of the ROM file to
													 * load
													 */
	private int romlist_orglength[] = new int[MAXLIST]; /* length backup */
	private int romlist_crc[] = new int[MAXLIST]; /* CRC of the ROM file */
	private boolean romlist_loaded[] = new boolean[MAXLIST]; /*
															 * is this ROM file
															 * already loaded ?
															 */
	private boolean romlist_cont[] = new boolean[MAXLIST]; /*
															 * is this ROM file
															 * is a continued
															 * ROM ?
															 */
	private int romlist_skip[] = new int[MAXLIST]; /*
													 * bytes to skip for this
													 * ROM file
													 */
	private int romlist_usetmp[][] = new int[MAXLIST][]; /*
														 * temporary memory to
														 * use to store the ROM
														 * file
														 */
	private boolean romlist_invert[] = new boolean[MAXLIST]; /*
															 * is this ROM file
															 * is to be inverted
															 * ?
															 */
	...
	private final void loadFromWeb(URL baseURL, boolean bParent) {
		...
		for (;;) {
			try {
				...
				System.out
						.println("ERROR loading ROM: error getting zip entry");
				System.out.println(e);
				for (;;)
					;
			}
				...
				if ((load_count != this.romlist_index) && (!bParent)) {
					for (i = 0; i < this.romlist_index; i++) {
						if (this.romlist_loaded[i] == false) {
							System.out.println(this.romlist_names[i] + "("
									+ Integer.toHexString(this.romlist_crc[i])
									+ ") not found !");
							try {
								jef.video.Console.TXT[jef.video.Console.cTXT] = this.romlist_names[i]
										+ "("
										+ Integer
												.toHexString(this.romlist_crc[i])
										+ ") not found !";
								jef.video.Console.update();
								jef.video.Console.cTXT = (jef.video.Console.cTXT + 1)
										% jef.video.Console.nTXT;
							} catch (Exception e) {
							}
						}
					}
			...
			for (i = 0; i < this.romlist_index; i++) {
				// if ((!this.romlist_loaded[i]) && (ze.getCrc() ==
				// (long)this.romlist_crc[i]) ) {
				if ((!this.romlist_loaded[i])
						&& ((this.romlist_crc[i] == 0) || (Long.toHexString(ze
								.getCrc()).equals(Integer
								.toHexString(this.romlist_crc[i]))))) {
					// if ((!this.romlist_loaded[i]) &&
					...
					try {
						while (toreadbytes > 0) {
							nreadbytes = zis.read(buffer, nbytes - toreadbytes,
									toreadbytes);
							if (nreadbytes == -1)
								break;
						...
						System.out
								.println("ERROR loading ROM: invalid ZIP file");
						for (;;)
							;
					}
