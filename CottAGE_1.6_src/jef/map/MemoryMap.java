...
public class MemoryMap implements ReadMap, WriteMap {
	...
	public int read(int address) {
		for (int i = 0; i < nrOfReadRegions; i++) {
			...
			// System.out.println(Integer.toHexString(r.from) + "-" +
			// Integer.toHexString(r.until));
			if (address >= r.from && address <= r.until) {
				return r.handler.read(address - r.from);
			}
		}
		// System.out.println("Unknown read at " +
	...
	public int[] findMemory(int from, int until) {
		for (int i = 0; i < nrOfReadRegions; i++) {
			...
			if (rr.from == from && rr.until == until) {
				if (rr.memory == null) {
					throw new RuntimeException("Memory for write region 0x"
							+ Integer.toHexString(from) + "-0x"
							+ Integer.toHexString(until) + " is null");
				}
				return rr.memory;
			}
		}
		throw new RuntimeException("Could not find memory for write region 0x"
				+ Integer.toHexString(from) + "-0x"
				+ Integer.toHexString(until));
	}
