...
public class GfxManager implements VideoConstants {
	...
	public final void init(GfxDecodeInfo gdi, int[] palette, int[] colorLookUp,
		...
		for (int n = 0; n <= w; n++) { // calculate tileMameOff (which is a hack
										// BTW) FIX THIS
			if ((gdi.gfx.offsX[n] & 7) > tileMameOff) {
				tileMameOff = gdi.gfx.offsX[n] & 7;
			}
		}
	}
	...
	public BitMap getTile(int tile, int color, int trans, int transcol) {
		...
		try {
			if (!valid[tile] || colorChanged[color]) {
				...
				tileImg[tile] = decode(tile, color, tileImg[tile], this.rot,
						this.flip, trans, transcol);
				valid[tile] = true;
			}
		} catch (Exception e) {
	...
	public void drawTile(BitMap target, int tile, int color, boolean flipx,
		...
		case VideoConstants.ROT0:
			getTile(tile, color, transparency, transcolor).toBitMap(target, x,
					y, flipx, flipy, owTrans);
			break;
		case VideoConstants.ROT90:
			getTile(tile, color, transparency, transcolor).toBitMap(target,
					(target.getWidth() - w - 1) - y, x, flipx, flipy, owTrans);
			break;
		case VideoConstants.ROT180:
			getTile(tile, color, transparency, transcolor).toBitMap(target,
					(target.getWidth() - w - 1) - x,
					(target.getHeight() - h - 1) - y, flipx, flipy, owTrans);
			break;
		case VideoConstants.ROT270:
			getTile(tile, color, transparency, transcolor).toBitMap(target, y,
					(target.getHeight() - h - 1) - x, flipx, flipy, owTrans);
			break;
		}
	}

	/**
	 * Notify me if something changed the color table. This marks the
	 * correspondent colorcode as changed.
	 */
	...
	/**
	 * Notify me if something changed the palette. This marks the correspondent
	 * colorcode as changed.
	 */
	public void changePalette(int paletteIndex, int argb) {
		palette[paletteIndex] = argb;
		try {
			colorChanged[(paletteIndex - gdi.colorOffset) >> gdi.gfx.planes] = true; // TO
																						// DO:
																						// this
																						// is
																						// not
			...
			// The color that is changed is not used by the layer that is using
			// this
	...
	/**
	 * Returns true if the given colorcode has changed. Can be handy to update
	 * the dirty rectangles buffer.
	 */
	...
	/**
	 * Resets all 'changed' flags. Call refresh() at last in a frame.
	 */
	 ...
	 * Decode a tile
	 */
	protected final BitMap decode(int tile, int col, BitMap tileImg, int rot,
			int flip, int trans, int transcol) {
		int byteOffset, bitOffset, realXOffset;
		int colorGroup = col * tileColors;
		int charAddress = offset + (tile % gdi.gfx.total)
				* (gdi.gfx.bytes >> 3);
		...
		for (int y = 0; y <= h; y++) {
			for (int x = 0; x <= w; x++) {
				byteOffset = (gdi.gfx.offsX[x]) & 0xfffff8; // offset within the
															// character in
															// bytes of this
															// pixel
				bitOffset = (gdi.gfx.offsX[x]) & 7; // offset within the
													// character in bits of this
													// pixel
				realXOffset = byteOffset + (tileMameOff - bitOffset);
				colorIndex = 0;

				try {
					for (int p = 0; p < gdi.gfx.planes; p++) { // This loop
																// assembles the
																// colorindex
						...
						if ((GFX[charAddress + byteOffset] & bit[bitOffset]) != 0) {
							colorIndex |= 1 << p; // if the read bit is set, set
													// the correct bit in the
													// colorindex
						}
					}
					...
					if (trans == TRANSPARENCY_NONE) {
						cc = palette[cc];
					} else if ((trans == TRANSPARENCY_PEN && colorIndex == transcol)
							|| (trans == TRANSPARENCY_COLOR && cc == transcol)) {
						cc = -1;
					} else {
						cc = palette[cc];
					}
					...
					case 0:
						...
						break;
					case 4:
						tileImg.setPixelFast(h - y, x, cc);
						break;
					case 8:
						tileImg.setPixelFast(w - x, h - y, cc);
						break;
					case 12:
						tileImg.setPixelFast(y, w - x, cc);
						break;
					}
				} catch (Exception e) {
				}
